<template>
    <div id="staffShift">
        ...
        <v-card class="mx-auto table-card v-card v-sheet theme--ligh">
            <template>
                <JqxGrid ref="gridStaffShift" :width="'100%'" :source="dataAdapter" :columns="columns" :pageable="true" :autoheight="true" :columnsresize="true" :editable="false"></JqxGrid>
            </template>
...
window.selectDocument = function(e) {
...
window.selectAllDocuments = function(e) {
...
export default{
    ...
    data() {
        ...
        return {
            ...
            columns: [
            {
                ...
                renderer: function () {
                    return '<div style="margin-left: 10px; margin-top: 12px;"><input type="checkbox" id="selectAll" onclick="selectAllDocuments(this)" /></div>';
                },
                cellsrenderer: function(row, columnfield, value, defaulthtml, columnproperties) {
                    return '<div style="margin-left: 10px; margin-top: 12px;"><input type="checkbox" class="selectCheckbox" onclick="selectDocument(this)" /></div>';
                }
            },
                {
                    text: this.$t('staffShift.tbEmployeeName'), datafield: 'Employees_Name', width: '23%', columntype: 'input',
                    renderer: function (row, column, value) {
                        return '<div style="text-align: center; margin-top: 8px;">' + row + '</div>';
                    },
                },
                {
                    text: this.$t('staffShift.tbShiftName'), datafield: 'ShiftName', width: '22%', columntype: 'input',
                    renderer: function (row, column, value) {
                        return '<div style="text-align: center; margin-top: 8px;">' + row + '</div>';
                    },
                },
                { 
                    text: this.$t('staffShift.tbStartDate'),datafield: 'StartDate', width: '17%', columntype: 'datetimeinput',  align:"center",
                    renderer: function (row, column, value) {
                ...
                { 
                    text: this.$t('staffShift.tbEndDate'),datafield: 'EndDate', width: '17%', columntype: 'datetimeinput',  align:"center",
                    renderer: function (row, column, value) {
                        return '<div id="jqxTimePicker" style="text-align: center; margin-top: 8px;">' + row + '</div>';
                    },
                    cellsrenderer: function (row, column, value) {
                ...
                {
                    text: this.$t('staffShift.tbRemark'), datafield: 'Remark', width: '21%', columntype: 'input',
                    renderer: function (row, column, value) {
                        return '<div style="text-align: center; margin-top: 8px;">' + row + '</div>';
                    },
                },
    ...
    methods: {
        ...
        // EditOrUpdateStaffShift(){
        //     this.staffShiftForm.StaffShiftDataForm = this.dataAdapter.records;
        //     this.staffShiftForm.Employees_id = this.form.id;
        //     this.referenceForm.post("api/edit-or-update-staff-shift", {
        //         headers: {
        //             Authorization: "Bearer " + sessionStorage.getItem("TOKEN"),
        //         },
        //     })
        ...
        editStaffShift(item) {
            ...
            this.dataAdapter._source.localdata = item.staffShift;
            if(this.$refs.gridStaffShift !== undefined) {
                this.$refs.gridStaffShift.updatebounddata('cell');
            }
            this.openDialog = true;
        ...
        async updateStaffShift() {
            ...
            this.form
            .post("/api/update-staff-shift/" + this.form.PKID, {
                headers: {
                    Authorization: "Bearer " + sessionStorage.getItem("TOKEN"),
                },
            })
