                    ...
                    { 
                        ...
                        createeditor: function (row, value, editor) {
                            ...
                            return editor.jqxDropDownList({ 
                                autoDropDownHeight: true, source: self.TimeInTimeOutArr, displayMember: 'TimeInTimeOut', valueMember: 'PKID', 
                            });
                        }
                    },
                ]
            ...
            ReadShiftDetail(){
                ...
                .then((respone) => {
                    for(var i = 0; i < respone.data.data.length; i++) {
                        if(respone.data.data[i].ShiftDetail.length > 0) {
                            if(respone.data.data[i].PKID == this.form.ShiftHeaderPKID) {
                                for(var j = 0; j < respone.data.data[i].ShiftDetail.length; j++) {
                                    var timeIn = respone.data.data[i].ShiftDetail[j].TimeIn;
                                    var timeOut = respone.data.data[i].ShiftDetail[j].TimeOut;
                                    var concatTimeInTimeOut = timeIn + "-" + timeOut;
                                    var objDataForm = {
                                        PKID: respone.data.data[i].ShiftDetail[j].PKID,
